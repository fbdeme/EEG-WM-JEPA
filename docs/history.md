...
### Phase 13: 학습 데이터 로딩 병목 분석 및 전략 확정
- **HF 스트리밍 병목 발견**: GPU 연산 0.27s/step vs 데이터 로딩 1.8s/step → GPU 가동률 15%
  - shard 전환 시 최대 48초 네트워크 대기 발생
  - 100 epochs 추정 57일 (full 데이터 기준)
- **로컬 디스크 방식 검토**: full 데이터 ~2.5TB → 이 서버(422GB) 수용 불가
- **더블버퍼 방식 채택**: 8 병렬 다운로드(1.8s/shard) > GPU 학습(2.6s/shard) → GPU가 병목 → GPU 100% 활용
  - 디스크 60GB만 사용 (버퍼 A/B 교체)
  - 로컬 2.5TB든 더블버퍼든 GPU가 병목이라 학습 시간 동일 (~10일/100ep)
- **학습 계획 확정**:
  - HP 튜닝: 로컬 300GB 부분 데이터, 10-20 epoch/실험, 실험당 2-4시간
  - Final 학습: 더블버퍼 full 데이터, 50-100 epoch, ~5-10일
- **실측 데이터**:
  - GPU compute: 0.27s/step (batch=512, A6000 48GB, 5.17GB VRAM)
...
| 2026-04-02 | 학습 데이터 로딩 병목 분석 → 더블버퍼 방식 채택 | HF 스트리밍 GPU 가동률 15%, 로컬은 디스크 부족 | 더블버퍼로 GPU 100% + 디스크 60GB |
